...
class DataLoader:
    ...
    def __init__(self, cell_path, tissue_path):
        ...
        if len(cell_fpath) == 0 and len(tissue_fpath) == 0:
            # Now search for individual 'jpg' files instead
            cell_fpath = sorted(
                [os.path.join(cell_path, f) for f in os.listdir(cell_path) if ".jpg" in f])
            tissue_fpath = sorted(
                [os.path.join(tissue_path, f) for f in os.listdir(tissue_path) if ".jpg" in f])
            if len(cell_fpath) > 0 and len(tissue_fpath) > 0:
                self.as_tif = False  # Stored as jpgs
            else:
                assert False
        ...
        if self.as_tif:
            ...
            assert len(cell_fpath) == len(tissue_fpath)

            self.cell_filepaths = cell_fpath
            self.tissue_filepaths = tissue_fpath
